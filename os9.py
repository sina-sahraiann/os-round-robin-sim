...
class Process:
    def __init__(self, process_id, cpu_times, io_times, priority, arrival_time, os, lock, ready_queue):
        ...
        self.current_wait_for_arrival = time.time()
    ...
    def run(self):
        ...
        while self.current_cpu_index < len(self.cpu_times):
            ...
            if self.currently_waiting > 0 and self.current_wait_for_arrival >= self.arrival_time:
                self.waiting_time += self.currently_waiting
                self.simulation_time += self.currently_waiting
                print(f"Process {self.process_id} waiting time is {self.waiting_time}s")
    ...
    def visualize(self):
        ...
        for idx, process in enumerate(self.processes):
            ...
            if process.completion_time is not None:
                ...
                gnt.annotate(f'Completion\n{completion_time:.2f}s', xy=(completion_time, 10 + idx * 10 + 5),
